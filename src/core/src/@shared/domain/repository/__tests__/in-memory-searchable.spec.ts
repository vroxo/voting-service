...
class StubInMemorySearchableRepository extends InMemorySearchableRepository<StubEntity>{
  ...
  protected applyFilter(items: StubEntity[], filter: string | null) {
...
describe("InMemorySearchableRepository Unit Tests", () => {
  ...
  describe("applyFilter method", () => {
    it('should no filter items when filter param is null', async () => {
      const items: StubEntity[] = [new StubEntity({ name: 'name value', price: 5 })]
  ...
  describe("search method", ()=> {
    it('should apply only paginate when other params are null', async () => {
      const entity: StubEntity = new StubEntity({ name: "a", price: 5 })
      const items = Array(16).fill(entity);
      repository.items = items;

      const result: SearchResult<StubEntity> = await repository.search(new SearchParams());
    ...
    it('should apply paginate and filter', async () => {
      ...
      let result: SearchResult<StubEntity> = await repository.search(
    ...
    it('should apply paginate and sort', async () => {
      ...
      for (const i of arrange) {
        const result: SearchResult<StubEntity> = await repository.search(i.params);   
